%% NPS data display (doesn't save data)
...
% to stop display, press "stop" on the bottom-left corner of the plot

sampleRate = 1.25e6; % sampling frequency, Hz
nfft = 2^12; % number of points to sample in PSD
updateFrequency = 1; % number of plot updates per second

setGlobalFs(sampleRate);
setGlobalNFFT(nfft);
setGlobalPSD(zeros(nfft/2+1, 1));
...
s.NumberOfScans = sampleRate/updateFrequency;
...
lh1 = addlistener(s,'DataAvailable', @plotDataSpectrum); % plot data in real time
